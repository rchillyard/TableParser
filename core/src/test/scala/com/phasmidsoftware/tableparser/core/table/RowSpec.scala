...
import com.phasmidsoftware.tableparser.core.parse.ParserException
import com.phasmidsoftware.tableparser.core.util.FPException
...
class RowSpec extends flatspec.AnyFlatSpec with should.Matchers {
  ...
  it should "apply(Int) correctly" in {
    val r: Row = Row(Seq("1", "2", "Junk"), Header.create("A", "B", "C"), 0)
  ...
  it should "apply(String) correctly" in {
    val f: String => Try[String] = Row(Seq("1", "2", "Junk"), Header.create("A", "B", "C"), 0)
