...
import com.phasmidsoftware.tableparser.core.parse.StdCellParsers.columnHelper
import org.scalatest.flatspec
import org.scalatest.matchers.should

class ColumnHelperSpec extends flatspec.AnyFlatSpec with should.Matchers {
  ...
  it should "convert correctly with format" in {
    val ch = columnHelper(ColumnHelper.camelCaseColumnNameMapper, Some("$x_$c"))
    ch.lookup(None, "facebookLikes") shouldBe "facebook_Likes"
    ch.lookup(Some("X"), "facebookLikes") shouldBe "X_facebook_Likes"
  }

  it should "convert correctly w/o format" in {
    val ch = columnHelper(ColumnHelper.camelCaseColumnNameMapper)
    ch.lookup(None, "facebookLikes") shouldBe "facebook_Likes"
    ch.lookup(Some("X"), "facebookLikes") shouldBe "facebook_Likes"
  }
}
