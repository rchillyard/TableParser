...
import com.phasmidsoftware.tableparser.core.util.{EvaluateIO, TryUsing}
...
class TableSpec extends flatspec.AnyFlatSpec with should.Matchers {
  ...
  it should "parse from File" in {
    ...
    matchIO(Table.parseFile(new File("core/src/test/resources/com/phasmidsoftware/tableparser/core/table/intPairs.csv"))) {
      case xt@HeadedTable(_, _) => xt.size shouldBe 2
    }
  }
