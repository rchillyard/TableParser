...
class CsvRenderersSpec extends AnyFlatSpec with should.Matchers {
  ...
  //    import IntPair._
  //    val iIty: IO[Table[IntPair]] = Table.parseFile(new File("src/test/resources/com/phasmidsoftware/table/intPairs.csv"))
  ...
  //
  //    val rty: IO[Table[DailyRaptorReport]] = for (r <- Table.parseResource(classOf[TableParserSpec].getResource("/raptors.csv"))) yield r
  ...
  //
  //    val rty: IO[Table[NestedRaptorReport]] = for (r <- Table.parseResource(classOf[TableParserSpec].getResource("/raptors.csv"))) yield r
  //    rty should matchPattern { case Success(HeadedTable(_, _)) => }
  //    val rt = rty.get
  //    rt.rows.size shouldBe 13
  //    import CsvGenerators._
