...
class ProjectsFuncSpec extends AnyFlatSpec with Matchers with Futures with ScalaFutures {
  ...
  private def checkResultIO[X](result: IO[X])(check: PartialFunction[X, Unit]): Unit = {
    import cats.effect.unsafe.implicits.global
    whenReady(result.unsafeToFuture(), Timeout(Span(1, Second)))(check(_))
  }
  ...
  //    import cats.effect.unsafe.implicits.global
  //    val pty: Future[RawTable] = parseResource("TeamProjectEncrypted.csv", classOf[ProjectsFuncSpec]).unsafeToFuture()
  ////    pty should matchPattern { case Success(HeadedTable(_, _)) => }
  //    whenReady(pty)
  //    val pt = pty.get
  //    pt.size shouldBe 1
